...
{
    /// <summary>
    /// IMPORTANT FIX for a problem independent of CritterSpawnPatch/CritterSpawnLimitPatch:
    ///
    /// The original code in UpdateSpawns() does NOT check spawnConfig.RespawnPopulationMax
    /// (the value CritterSpawnPatch overrides) for the respawn branch, but instead:
    ///
    ...
    ///     }
    ///
    /// -- i.e. only on the ONE-TIME transition from WasAllowedToSpawn false -> true. For critter
    ...
    /// CritterSpawnPatch changes spawnConfig.RespawnPopulationMax afterwards (e.g. because the
    /// player raises a critter's SpawnChance in the config menu), the entry.RespawnPopulationMax
    /// value that actually matters for the respawn roll stays frozen at whatever it was back then.
    ///
    /// Concretely observed: a critter that used to run with MaxLimit=0 (SpawnChance=0) has
    ...
    /// current effective MaxLimit for every critter with an active override, before EVERY
    ...
    /// on every tick. CritterSpawnPatch only sets those values ONCE, in the Prefix of Setup() -
    ...
    /// the spawnConfig object, not from the config. Result: a freshly enabled critter (e.g. Gnasher
    ...
    /// on EVERY tick - on the RespawnRarity asset already cloned per-critter by CritterSpawnPatch, so
    /// there is still no cross-talk between critter sharing the same rarity tier.
    /// </summary>
    [HarmonyPatch]
    public static class CritterRespawnPopulationMaxSyncPatch
    {
        [HarmonyTargetMethod]
        private static MethodBase TargetMethod()
        {
            Plugin.LogDebug("[CritterRespawnPopulationMaxSyncPatch] Resolving TargetMethod(): EntityBasicCritterSpawner.UpdateSpawns()");
            MethodBase method = AccessTools.Method(typeof(EntityBasicCritterSpawner), "UpdateSpawns");

            if (method == null)
                Plugin._logger.LogError("[CritterRespawnPopulationMaxSyncPatch] Could NOT find UpdateSpawns()! The sync patch will have no effect.");
            else
                Plugin.LogDebug($"[CritterRespawnPopulationMaxSyncPatch] TargetMethod resolved: {method.DeclaringType?.FullName}.{method.Name}");
        ...
        private static void Prefix(object __instance)
        {
            ...
            {
                Plugin._logger.LogError($"[CritterRespawnPopulationMaxSyncPatch] Error while syncing entry.RespawnPopulationMax: {ex}");
            }
        }
        ...
        private static void SyncPersistedRespawnPopulationMax(object spawnerInstance)
        {
            ...
            {
                Plugin.LogDebug("[CritterRespawnPopulationMaxSyncPatch] Field 'persistence' not found, skipping sync.");
                return;
            }
            ...
            if (persistence == null)
            {
                // During the very first Setup() pass (Prefix of CritterSpawnPatch), 'persistence'
                ...
                Plugin.LogDebug("[CritterRespawnPopulationMaxSyncPatch] 'persistence' is null, skipping sync.");
                return;
            }
            ...
            if (entries == null)
            {
                Plugin.LogDebug("[CritterRespawnPopulationMaxSyncPatch] 'persistence.Entries' is null, skipping sync.");
                return;
            }
            ...
            {
                Plugin._logger.LogWarning("[CritterRespawnPopulationMaxSyncPatch] No matching 'FindOrCreate' method found on 'Entries', skipping sync.");
                return;
            }
            ...
            {
                Plugin.LogDebug("[CritterRespawnPopulationMaxSyncPatch] Field 'spawnConfigs' not found, skipping sync.");
                return;
            }
            ...
            // once per critter, since it doesn't change during the loop.
            ...
            foreach (object spawnConfig in configList)
            {
                ...
                bool overrideActive = CritterConfigManager.OverrideCritterConfigs.TryGetValue(assetName, out var overrideEntry) && overrideEntry.Value;

                if (!overrideActive)
                {
                    // Not a known critter, or override disabled. If this SpawnConfig was never
                    ...
                    // otherwise the critter can never respawn again with its real defaults.
                    if (CritterOriginalValueStore.HasSnapshot(spawnConfig))
                    {
                ...
                {
                    Plugin.LogDebug($"[CritterRespawnPopulationMaxSyncPatch] '{assetName}': no matching 'FindOrCreate' overload found for parameter type '{itemAssetObj.GetType().FullName}', skipping.");
                    continue;
                }
                ...
                if (currentPersisted != maxLimit)
                {
                    ...
                    Plugin.LogDebug($"[CritterRespawnPopulationMaxSyncPatch] '{assetName}': synced entry.RespawnPopulationMax (persisted): {currentPersisted} -> {maxLimit}.");
                }

                // --- FIX 3: persisted entry.SpawnCount desync ---
                // The persisted entry.SpawnCount is meant to track how many critter of this type
                // currently exist, but it is never decremented when CritterSpawnLimitPatch blocks a
                // physical spawn (e.g. while MaxLimit was temporarily 0 due to SpawnChance=0).
                // If a critter that was already counted gets removed/never re-instantiated under
                ...
                // whenever it's too high - never upward, so we only correct the "phantom critter"
                ...
                // These values are only set once by CritterSpawnPatch, during Setup(). If the
        ...
        /// <summary>
        /// Reverts a previously-overridden critter back to its true original values: the persisted
        ...
        {
            if (!CritterOriginalValueStore.TryGet(spawnConfig, out var original))
                return;
            ...
            if (findOrCreate != null)
            {
                ...
                if (entry != null)
                {
                    ...
                    if (currentPersisted != originalPersisted)
                    {
                        ...
                        Plugin.LogDebug($"[CritterRespawnPopulationMaxSyncPatch] '{assetName}': override disabled, restored entry.RespawnPopulationMax (persisted): {currentPersisted} -> {originalPersisted}.");
                    }
            ...
            // --- Original RespawnRarity asset reference (undoes the per-critter clone) ---
            ...
            if (original.RespawnRarity != null && !ReferenceEquals(currentRarity, original.RespawnRarity))
            {
                ...
                Plugin.LogDebug($"[CritterRespawnPopulationMaxSyncPatch] '{assetName}': override disabled, restored original RespawnRarity asset ('{(currentRarity as UnityEngine.Object)?.name ?? "null"}' -> '{(original.RespawnRarity as UnityEngine.Object)?.name ?? "null"}').");
            }
        ...
        /// <summary>
        /// Corrects a desynced entry.SpawnCount downward to match the number of critter actually
        ...
        private static void SyncSpawnCountToInstantiated(IDictionary spawnEntries, object spawnConfig, object entry, string assetName)
        {
            ...
            {
                Plugin.LogDebug($"[CritterRespawnPopulationMaxSyncPatch] ({assetName}) 'spawnEntries' not found, skipping SpawnCount desync check.");
                return;
            }
            ...
            if (currentSpawnCount > instantiatedCount)
            {
                ...
                Plugin.LogDebug($"[CritterRespawnPopulationMaxSyncPatch] '{assetName}': corrected desynced entry.SpawnCount (persisted): {currentSpawnCount} -> {instantiatedCount} (matches actual InstantiatedSpawns).");
            }
        }
        ...
        /// current config. Operates on the RespawnRarity asset already cloned per-critter by
        /// CritterSpawnPatch, so there is no cross-talk between critter sharing the same rarity tier.
        /// </summary>
        private static void SyncLiveSpawnConfigValues(object spawnConfig, string assetName, int maxLimit)
        {
            int chancePercent = CritterConfigManager.SpawnChanceConfigs.TryGetValue(assetName, out var chanceCfg)
                ? chanceCfg.Value
                : 0;
            ...
            // Live values on the (per-critter cloned) RespawnRarity asset.
            object respawnRarity = ReflectionHelpers.GetMemberValue(spawnConfig, "RespawnRarity");
            if (respawnRarity == null)
            {
                Plugin.LogDebug($"[CritterRespawnPopulationMaxSyncPatch] '{assetName}': RespawnRarity is null, skipping live sync of chance/interval.");
                return;
            }
        ...
        private static void SetLiveFloatProperty(object obj, string propName, float newValue, string context)
        {
            ...
            if (prop == null)
            {
                Plugin.LogDebug($"[CritterRespawnPopulationMaxSyncPatch] ({context}) Property '{propName}' not found on type '{obj.GetType().FullName}'.");
                return;
            }

            if (!prop.CanWrite)
            {
                Plugin.LogDebug($"[CritterRespawnPopulationMaxSyncPatch] ({context}) Property '{propName}' on type '{obj.GetType().FullName}' is not writable.");
                return;
            }
            ...
            Plugin.LogDebug($"[CritterRespawnPopulationMaxSyncPatch] ({context}) Live {obj.GetType().Name}.{propName}: {oldValue} -> {newValue}");
        }

        /// <summary>
        /// Sets RespawnRarity.RespawnIntervalMinutes while accounting for the property's actual
        /// runtime type (int or float), analogous to SetPropertyValueTypedLogged in CritterSpawnPatch.
        /// </summary>
        private static void SetLiveTypedIntervalMinutes(object respawnRarity, int newValue, string context)
        {
            ...
            if (prop == null)
            {
                Plugin.LogDebug($"[CritterRespawnPopulationMaxSyncPatch] ({context}) Property 'RespawnIntervalMinutes' not found on type '{respawnRarity.GetType().FullName}'.");
                return;
            }

            if (!prop.CanWrite)
            {
                Plugin.LogDebug($"[CritterRespawnPopulationMaxSyncPatch] ({context}) Property 'RespawnIntervalMinutes' on type '{respawnRarity.GetType().FullName}' is not writable.");
                return;
            }
            ...
            {
                Plugin._logger.LogWarning($"[CritterRespawnPopulationMaxSyncPatch] ({context}) Unexpected type for 'RespawnIntervalMinutes': {oldValueObj?.GetType().FullName ?? "null"}. Setting as int anyway.");
                typedNewValue = newValue;
            }

            prop.SetValue(respawnRarity, typedNewValue);
            Plugin.LogDebug($"[CritterRespawnPopulationMaxSyncPatch] ({context}) Live RespawnRarity.RespawnIntervalMinutes: {oldValueObj} -> {typedNewValue}");
        }
    }
}